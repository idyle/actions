...
import { uploadFile, deleteFile, listFiles, deleteFiles, deployFiles, makeFrontendPublic, setMetadata } from './operations/storage.js';
...
export const createFrontend = async (folderPath, websiteName) => {
    ...
    console.info('STEP 1 of 8: Beginning instance creation...');
    const instance = await createFrontendInstance(websiteName);
    if (!instance) return console.error('Instance creation failed!', instance);

    console.info('STEP 2 of 8: Tracking instance creation...');
    const trackedInstance = await awaitInstance(instance?.name);
    if (!trackedInstance) return console.error('Instance tracking failed!', trackedInstance);

    console.info('STEP 3 of 8: Beginning mapping creation...');
    const mapping = await createMapping(websiteName, instance?.latestResponse?.targetLink);
    if (!mapping) return console.error('Mapping creation failed!', mapping);

    console.info('STEP 4 of 8: Beginning file retrieval...');
    const currentFiles = await listFiles();
    if (!currentFiles) return console.error('File retrieval failed!', currentFiles);

    console.info('STEP 5 of 8: Beginning file clean-up...');
    const deletedFiles = await deleteFiles(currentFiles);
    if (!deletedFiles) return console.error('File clean-up failed!', deletedFiles);

    console.info('STEP of 6 of 8: Beginning file upload...');
    const uploadedFiles = await deployFiles(folderPath, (await listFolder(folderPath)));
    if (!uploadedFiles) return console.error('FIle upload failed!', uploadedFiles);

    console.info('STEP 7 of 8: Beginning public access...');
    const access = await makeFrontendPublic();
    if (!access) return console.error('Public access failed!', access);

    console.info('STEP 8 of 8: Beginning metadata set...');
    const metadata = await setMetadata({ website: { mainPageSuffix: 'index.html', NotFoundPage: 'index.html' }});
...
export const updateFrontend = async (folderPath) => {
    console.time();
